...
- **すぐに使える** — サンプルの Live2D モデル + 無料のクラウド TTS（edge-tts）+ 自動ダウンロードされる音声認識（ASR）モデル（約 1GB。初回起動時に一度だけダウンロードされ、数分かかります）が同梱されています。あなたは LLM を差し込むだけです。
...
> Warashi は **体と顔** です — アバター、声、記憶。実際に考えて話す **頭脳** は、*あなた自身* が用意する別の AI です。セットアップウィザードの中で設定できるので、いま全部決めておく必要はありません。選択肢は、簡単な順に次のとおりです。
> - **（いちばん簡単・おすすめ）Ollama Cloud の無料プランを使う** — API キー不要、課金不要、高性能なパソコンも不要です。無料の **[Ollama](https://ollama.com)** アプリをインストールし、`ollama signin` で無料アカウント（クレジットカード不要）を作れば、`gpt-oss:20b-cloud` のような実力のあるクラウドモデルが使えます。Ollama のサーバー上で動くので、あなたのパソコンが速い必要はありません。**$0** ですが、軽い利用向けの上限があります（下の方法 C を参照）。
...
### 方法 C — Ollama 経由でクラウドモデルを使う（いちばん簡単・おすすめ、例：`gpt-oss:20b-cloud`）

**これが、非技術者にとって API キー不要のいちばん簡単な道です。** Ollama は、Ollama のサーバー上で動く強力な **クラウド** モデルをプロキシするので、あなたのパソコンが速い必要はありません。しかも無料プランは **$0、クレジットカード不要** です。手順：

1. [ollama.com/download](https://ollama.com/download) から Ollama をインストールします。
2. ターミナルで **`ollama signin`** を実行し、無料アカウント（カード不要）を作ります。
3. **`ollama pull gpt-oss:20b-cloud`** を実行して、おすすめモデルを取得します。
4. Warashi の **Ollama モード** で、モデル名 `gpt-oss:20b-cloud` を入力します。

pull したクラウドモデルは、Warashi がすでに使っている同じローカル Ollama エンドポイント（OpenAI 互換、`localhost:11434`）から提供されるので、Warashi の既存の Ollama モードがそのまま使えます。

**おすすめモデル：** `gpt-oss:20b-cloud` — 「Low Usage」表示で、無料の上限にいちばん耐えます。より強力な `qwen3.5:cloud` や `minimax-m3:cloud` は性能は上ですが、上限の消費が速くなります。

> **無料プランについて正直に：** これは本当に **$0、クレジットカード不要、無料アカウントだけ** で使えます — ただし *軽い利用向け* のプランです。Ollama では **一度に 1 つのクラウドモデル** まで、セッション上限は約 5 時間ごと、週次上限は 7 日ごとにリセットされます。Ollama は正確な数値を **公開していない** ので、たくさん話すと一時的に上限に達し、リセットまで待つことがあります。推論は **Ollama のサーバー上（あなたのマシンではなく）** で動くため、完全に非公開にしておきたい内容は送らないでください。
