...
- **설치하자마자 작동** — 샘플 Live2D 모델 + 무료 클라우드 TTS(edge-tts) + 자동으로 내려받는 소형 ASR 모델을 기본 제공합니다. 당신은 LLM만 연결하면 됩니다.
...
> Warashi는 **몸과 얼굴**입니다 — 아바타, 음성, 기억. 실제로 생각하고 말하는 **두뇌**는 *당신*이 직접 준비하는 별도의 AI입니다. 두 가지 방법 중 하나를 고르면 되고, 설정 마법사를 진행하면서 설정할 수 있습니다(지금 당장 전부 결정할 필요는 없습니다):
...
설치하자마자 기본으로 제공되는 **mao** 샘플 Live2D 모델과 **edge-tts**(무료 클라우드 음성, GPU 불필요)를 사용합니다. 첫 실행 때 소형 음성-텍스트 변환(STT) 모델도 자동으로 내려받습니다.
...
### 옵션 C — Ollama를 통한 클라우드 모델 (예: `gpt-oss:cloud`)

Ollama는 특정 **클라우드** 모델을 프록시할 수도 있습니다. **Ollama 모드에서는 모델 이름을 직접 입력할 수 있으며**, `:cloud`로 끝나는 클라우드 이름도 포함됩니다(예: `gpt-oss:cloud`).
